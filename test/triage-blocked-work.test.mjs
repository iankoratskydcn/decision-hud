...
assert.match(source, /cliExec\(\['kanban', '--board', boardSlug, 'list', '--status', 'blocked', '--json'\]\)/)
assert.match(source, /kanban', '--board', boardSlug, 'show', task\.id, '--json'/)
assert.match(source, /'--graph', JSON\.stringify\(graph\)/)
